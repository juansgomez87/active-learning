...
class Recommender():
    ...
    def run(self):
        ...
        this_test = StandardScaler().fit_transform(this_data.loc[:, 'F0final_sma_stddev':'mfcc_sma_de[14]_amean'])
        ...
        for i, mod_fn in enumerate(self.mod_list):
            ...
            mod = joblib.load(mod_fn)
            y_probs = mod.predict_proba(this_test)
...
if __name__ == "__main__":
    # usage: python3 get_recommendations.py -i USER_ID -r RECOMMENDATIONS
    # example: python3 get_recommendations.py -i 827 -r africa
    start = time.time()
    ...
    rec = Recommender(path_to_models, args.recs)
